...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, norm_type='BN',stride=1):
        super(BasicBlock, self).__init__()
        if norm_type == 'BN':
            self.norm = nn.BatchNorm2d(planes)
            norm_layer = nn.BatchNorm2d(self.expansion*planes)
        else:
            self.norm = nn.GroupNorm(1,planes)
            norm_layer=nn.GroupNorm(1,self.expansion*planes)  
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks,norm_type='BN', num_classes=10):
